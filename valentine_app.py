...
def send_mail_from_secrets(subject: str, body: str):
    ...
    to_email = "target@gmail.com"    # or JSON list string: ["a@gmail.com","b@gmail.com"]
    use_tls = true
    """
    try:
...
# =============================
# Build tiny custom component at runtime
# =============================
COMPONENT_DIR = Path(__file__).parent / "_valentine_component"
...
index_html = textwrap.dedent(
    ...
    <body>
      ...
      <script>
        ...
        function setFrameH() {
          Streamlit.setFrameHeight(document.documentElement.scrollHeight + 8);
        }
        ...
        function sendYesEventOnce() {
          if (sentEvent) return;
          sentEvent = true;
          const token = `${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
          Streamlit.setComponentValue({ event: "YES_CLICK", token: token });
        }
        ...
        function onRender(event) {
          // If Python wants to disable after sent, can pass arg.
          const args = event?.detail?.args || {};
          if (args.mail_sent === true) {
            accepted = true;
            sentEvent = true;
            render();
          }
        ...
        yesBtn.addEventListener("click", () => {
          ...
          floatBurst();
          render();
          sendYesEventOnce(); // <-- reliable backend signal
        });
...
(COMPONENT_DIR / "index.html").write_text(index_html, encoding="utf-8")

valentine_component = components.declare_component(
    "valentine_component",
...
st.markdown(
    ...
    .block-container { max-width: 100% !important; padding-top: 0.2rem !important; }
    </style>
    """,
...
event = valentine_component(
    key="valentine_ui",
    default={"event": None, "token": None},
    mail_sent=st.session_state.mail_sent,
)

if isinstance(event, dict) and event.get("event") == "YES_CLICK":
    token = str(event.get("token") or "")
    if token and token not in st.session_state.sent_tokens:
        ...
        st.session_state.sent_tokens.add(token)
