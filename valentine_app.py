...
# -----------------------------
# Main interactive UI (single container flow)
# -----------------------------
html = r"""
<!doctype html>
<html>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .ack-btn{
      ...
      color: #fff;
      cursor: pointer;
      background: linear-gradient(135deg,#ff3b3b,#ff6b6b,#ff8e53);
...
<body>
  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      <div id="ackWrap" class="ack-wrap">
        <div class="ack-note">click here to get valintine outing date😝</div>
        <button id="ackBtn" class="ack-btn">I acknowledge ✅</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    ackBtn.addEventListener("click", () => {
      ...
      ackBtn.textContent = "Sending your outing date mail... 💌";
      triggerMailSend();
    });
...
# Main interactive UI (inside iframe)
# -----------------------------
html = r"""
<!doctype html>
<html>
...
<body>
  ...
  <script>
    ...
    yesBtn.addEventListener("click", () => {
      ...
      floatBurst();
      render();
      // NOTE: mail trigger intentionally removed (iframe signal issue workaround)
    });
...
# -----------------------------
# Reliable native Streamlit mail trigger
# -----------------------------
st.markdown("### Final step 💌")
st.caption("After she clicks YES above, press this acknowledgment button to send the email.")
...
with col1:
    if st.button(
        "I acknowledge ✅ Send our date email",
        type="primary",
        use_container_width=True,
        disabled=st.session_state.mail_sent
    ):
        subject, body = get_email_content()
        ok, info = send_mail_from_secrets(subject, body)
        if ok:
